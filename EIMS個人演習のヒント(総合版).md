...
```java
// ★文字列を複数入れる配列を作ります。
// 左側には「文字列の配列」を表す型を書きます。
____ keywords = { "class", "int", "boolean", "if", "while" };
```
...
```java
// ★問題数と文字列をつなげて表示します。
// ____ には、配列の要素数を取得する書き方を入れます。
System.out.println(keywords.____ + "個の問題があります");
```
...
```java
// ★配列の中から、文字列を1つずつ取り出します。
// ____ には、取り出し元になる配列名を書きます。
for (String word : ____) {
...
```java
// ★1000で割った余りを使って、ミリ秒部分だけを取り出します。
// ____ には、余りを求める演算子を書きます。
long msec = score ____ 1000;       // ① 1000で割った余り → ミリ秒部分

// ★1000で割って、「何秒分あるか」を求めます。
// ____ には、割り算の演算子を書きます。
long secScore = score ____ 1000;   // ② 何秒分あるか
...
// ★時間が0のときは「0時間」と表示しないようにします。
// ____ には、0より大きいかを判定する条件を書きます。
if (____) {
    strScore += hour + "時間";
}

// ★分も同じように、0より大きいときだけ追加します。
// ____ には、分を表示するかどうかの条件を書きます。
if (____) {
    strScore += min + "分";
}
...
// ★4回繰り返して、値が小さくなっても正しく表示できるか確認します。
// ____ には、4回繰り返すための条件を書きます。
for (int i = 0; ____; i++) {
...
// ★比較対象の文字列を用意します。
// 配列の中にある文字列を1つ選んで入れると、等しい場合の確認ができます。
String input = "____";
```
...
```java
// ★文字列の中身を比較します。
// ____ には、Stringの中身を比較するメソッド名を書きます。
if (input.____(word)) {
...
for (String word : keywords) {
    ...
    // ★input と word の文字列が同じかを判定します。
    // == ではなく、文字列比較用のメソッドを使います。
    if (input.____(word)) {
        System.out.println("等しいです");
    } else {
        System.out.println("等しくないです");
...
```java
// ★フィールドはクラスの外から直接変更されないようにします。
// ____ には、カプセル化でよく使うアクセス修飾子を書きます。
____ String name;
____ long longScore;
____ String date;
____ String score;
...
public Score(String name, long longScore, String date) {
    ...
    // ____ には、演習1.2から移した変換メソッドの呼び出しを書きます。
    this.score     = ____(longScore);
}
```
...
```java
// ★Scoreクラスの中だけで使う補助メソッドにします。
// ____ には、クラス外から呼ばせないためのアクセス修飾子を書きます。
____ static String getStringScore(long score) {
...
public String toString() {
    // ★カンマと空白の位置を実行例と同じに
    // ____ には、表示用スコアを持つフィールド名を書きます。
    return name + ", " + ____ + ", " + date;
}
```
...
```java
// ★Systemクラスの、現在時刻をミリ秒で返すメソッドを呼び出します。
// ____ には、APIドキュメントで見つけたメソッド名を書きます。
long now = System.____();
```
...
```java
// ★開始時刻と終了時刻は、ミリ秒の大きな数値になるので long 型にします。
// ____ には、外部から直接変更されないようにするアクセス修飾子を書きます。
____ long start;
____ long end;
```
...
public void start() {
    // ★開始時刻をミリ秒で取得
    // ____ には、現在時刻をミリ秒で取得するメソッド呼び出しを書きます。
    start = System.____();
}

public void stop() {
    // ★終了時刻をミリ秒で取得
    // start() と同じ考え方で、終了時刻を保存します。
    end = System.____();
}
```
...
// ★測定したい処理の直前で、開始時刻を保存します。
s.____();

// ここに、時間を測りたい処理を書く

// ★測定したい処理の直後で、終了時刻を保存します。
s.____();

// ★かかったミリ秒を取得して表示します。
System.out.println(s.____() + "ミリ秒");
```
...
while (true) {
    ...
    // ★KBクラスのメソッドを使って、キーボードから1行読み取ります。
    // ____ には、KBクラスに作成した入力用メソッド名を書きます。
    String input = KB.____();

    // ★"exit" と比較するときは equals() を使う
    // ____ には、ループを終了するためのキーワードを書きます。
...
```java
// ★ファイルから読み込んだ複数行を返すので、戻り値は文字列リストにします。
// ____ には、文字列を複数入れるリストの型を書きます。
public static ____ read(String filename) {

    // ★読み込んだ行を入れていく空のリストを作ります。
    // ____ には、上の戻り値と同じリストの型を書きます。
    ____ list = new ArrayList<>();

    // ★try-with-resources で自動クローズ
    // ____ には、ファイル名から文字を読むためのクラスを書きます。
    try (BufferedReader br = new BufferedReader(new ____(filename))) {
        String line;
...
```java
// ★TypingFileクラスのstaticメソッドを呼び出して、ファイル内容を受け取ります。
// ____ には、ファイル読み込み用のクラス名を書きます。
ArrayList<String> data = ____.read("keywords.txt");
...
ここで使っている `"now"` は、Javaが自動で現在日時に変換してくれる特別な言葉ではありません。
ダブルクォーテーションで囲まれているので、ただの文字列です。

| 書き方 | 意味 |
| --- | --- |
| `"now"` | Javaの文字列。中身は `now` という4文字だけ |
| `System.currentTimeMillis()` | 現在時刻をミリ秒で取得するJavaのメソッド |
| `now()` | MySQLで現在日時を入れる関数 |

演習3.3では、`Score` クラスのコンストラクタが「名前・スコア・日付文字列」の3つを必要とするため、第3引数に何か文字列を渡す必要があります。
ただし、この画面では日付を表示しないため、仮の値として `"now"` を入れているだけです。
`"now"` と書いたから現在日時が入るわけではない、という点だけ注意してください。
...
`Score` オブジェクト生成時の第3引数には、任意の文字列（`"now"` など）を入れて大丈夫です。
ここでの `"now"` は、Java側で使う仮の日付文字列です。
本当にDBへ登録される日時は、演習5.3で作った `insert` メソッドのSQLにある MySQL の `now()` 関数で決まります。

混乱しやすいので、次のように分けて考えてください。

| 場所 | 書き方 | 役割 |
| --- | --- | --- |
| Java側 | `"now"` | `Score` を作るために一時的に渡す文字列 |
| SQL側 | `now()` | DBの `datetime` 列に現在日時を登録する関数 |

つまり、`new Score(..., "now")` の `"now"` がDBにそのまま入るわけではありません。
`dao.insert(score)` を呼び出すと、SQLの `now()` が実行され、その時点の現在日時が `datetime` に登録されます。
