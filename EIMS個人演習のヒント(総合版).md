...
`12345 / 1000` は `12` です。これは「全体を秒に直した値」になります。
...
long secScore = score / 1000;   // ② 全体を秒に直した値

long sec = secScore % 60;       // ③ 60で割った余り → 秒部分
long minScore = secScore / 60;  // ④ 全体を分に直した値
...
たとえば、`secScore` は「全体を秒に直した値」です。`minScore` は「全体を分に直した値」です。
...
`Score` クラスで考えるものは、次の4つのフィールドです。コンストラクタの引数は3つですが、表示用の `score` はコンストラクタの中で作ります。
