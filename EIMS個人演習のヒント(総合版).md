...
演習2.1は、演習1.1のコードを土台にすると作りやすいです。

| コピー元 | コピーしてよい部分 | 演習2.1で変える部分 |
| --- | --- | --- |
| `Ex1_1` | `String[] keywords = { ... };` | そのまま使ってよい |
| `Ex1_1` | `keywords.length` を使った問題数表示 | そのまま使ってよい |
| `Ex1_1` | `for (String word : keywords)` | ループの中で、表示だけでなく比較も行う |

つまり、演習1.1の「配列を用意して、1つずつ取り出す」部分はそのまま活かせます。

新しく追加するのは、`String input = "boolean";` と、`input.equals(word)` による比較です。
...
演習2.2では、演習1.2で作った時間変換メソッドを再利用します。

| コピー元 | コピーしてよい部分 | 演習2.2で置く場所 |
| --- | --- | --- |
| `Ex1_2` | `getStringScore(long score)` メソッド | `Score` クラスの中 |
| `Ex1_2` | `msec`, `secScore`, `sec`, `minScore`, `min`, `hour` の計算 | `getStringScore` の中 |
| `Ex1_2` | `hour > 0` などの条件分岐 | `getStringScore` の中 |

ただし、`main` メソッドはコピーしません。

演習2.2で必要なのは、時間を文字列に変換する `getStringScore` メソッドだけです。

また、`getStringScore` は `Score` クラスの内部で使う補助メソッドなので、`private static` にしておくとよいです。
...
演習2.3の `Stopwatch` クラス本体は、新しく作るクラスです。

そのため、`Stopwatch` クラスへそのままコピーする既存クラスはありません。

ただし、動作確認用の `Ex2_3` では、演習1.1で使った `for` 文の考え方を使います。

| 参考にするもの | 使う考え方 | 演習2.3での使い方 |
| --- | --- | --- |
| `Ex1_1` | 繰り返し処理 | `for` 文で0から999まで表示する |
| 演習2.3で新規作成 | `Stopwatch` クラス | 表示処理の前後で `start()` と `stop()` を呼ぶ |

つまり、ここでは「既存コードを丸ごとコピーする」というより、「演習1.1で学んだ繰り返し処理を、時間計測の対象にする」と考えると分かりやすいです。
...
```java
while (keywords.____() > 0) {
    // ★問題が何問残っているかを表示する
    System.out.println("あと" + keywords.____() + "問");

    // ★現在の問題を表示して、入力を1行受け取る
    System.out.print(keywords.____(____) + "> ");
    String line = KB.readLine();

    // ★入力内容と、現在の問題が一致しているかを判定する
    if (line.equals(keywords.____(____))) {
        // ★正解した問題をリストから取り除く
...
`keywords` は「まだ残っている問題のリスト」、`line` は「受講者が入力した文字列」と考えると整理しやすくなります。

迷ったときは、次の順番で考えてください。

| やりたいこと | 見る対象 | 考えること |
| --- | --- | --- |
| 残り問題数を知りたい | `keywords` | リストの個数を調べるにはどうするか |
| 今の問題を表示したい | `keywords` | リストの先頭要素を取り出すにはどうするか |
| 正誤判定をしたい | `line` と現在の問題 | 文字列の中身を比較するにはどうするか |
| 正解した問題を消したい | `keywords` | リストから指定位置の要素を削除するにはどうするか |
| 次の出題順を変えたい | `keywords` | リスト全体をシャッフルするにはどうするか |
