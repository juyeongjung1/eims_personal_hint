...
.admin-area {{
  background: #fdfefe; border: 1px solid var(--line); border-radius: 8px;
  margin: 34px 0 0; padding: 18px; color: var(--ink);
}}
...
.admin-button.primary {{ background: var(--green); border-color: var(--green); color: white; }}
.admin-login, .admin-panel {{ display: none; margin-top: 14px; }}
.admin-login.active, .admin-panel.active {{ display: block; }}
.admin-login input {{
  width: min(260px, 100%); height: 38px; border: 1px solid var(--line); border-radius: 6px;
  padding: 0 10px; margin-right: 8px;
}}
.admin-message {{ color: var(--red); font-size: 13px; margin: 8px 0 0; }}
.schedule-grid {{
  display: grid; grid-template-columns: repeat(5, minmax(120px, 1fr)); gap: 10px; margin: 10px 0 14px;
}}
.schedule-grid label {{
  display: grid; gap: 5px; font-weight: 700; color: #2c393b; font-size: 13px;
}}
.schedule-grid input {{
  height: 38px; border: 1px solid var(--line); border-radius: 6px; padding: 0 8px; background: white;
}}
.release-note {{
...
@media (max-width: 860px) {{
  ...
  .admin-login input {{ display: block; margin: 0 0 8px; }}
...
<section class="admin-area" aria-label="管理者設定">
  <div class="admin-actions">
    <button type="button" class="admin-button" id="adminLoginButton">管理者でログイン</button>
    <span id="scheduleSummary"></span>
  </div>
  <div class="admin-login" id="adminLogin">
    <input id="adminPassword" type="password" autocomplete="current-password" placeholder="管理者パスワード">
    <button type="button" class="admin-button primary" id="adminAuthButton">ログイン</button>
    <p class="admin-message" id="adminMessage"></p>
  </div>
  <div class="admin-panel" id="adminPanel">
    <div class="schedule-grid">
      <label>演習1<input type="time" data-release-input="ex1"></label>
      <label>演習2<input type="time" data-release-input="ex2"></label>
      <label>演習3<input type="time" data-release-input="ex3"></label>
      <label>演習4<input type="time" data-release-input="ex4"></label>
      <label>演習5<input type="time" data-release-input="ex5"></label>
    </div>
    <div class="admin-actions">
      <button type="button" class="admin-button primary" id="saveSchedule">公開時刻を保存</button>
      <button type="button" class="admin-button" id="clearSchedule">公開時刻をすべて解除</button>
...
const scheduleInputs = Array.from(document.querySelectorAll('[data-release-input]'));
...
const scheduleKey = 'eimsHintReleaseSchedule';
const adminHash = 'c775e7b757ede630cd0aa1113bd102661ab38829ca52a6422ab782862f268646';
let releaseSchedule = loadSchedule();
let activeGroup = 'all';
...
function saveScheduleValues() {{
  releaseSchedule = Object.fromEntries(
    scheduleInputs
      .map(input => [input.dataset.releaseInput, input.value])
      .filter(([, value]) => value)
  );
  localStorage.setItem(scheduleKey, JSON.stringify(releaseSchedule));
...
function hydrateScheduleForm() {{
  scheduleInputs.forEach(input => {{
    input.value = releaseSchedule[input.dataset.releaseInput] || '';
  }});
}}
...
function updateScheduleSummary() {{
  const items = Object.entries(releaseSchedule)
    .filter(([, time]) => time)
    .map(([group, time]) => `演習${{group.replace('ex', '')}} ${{time}}`);
  scheduleSummary.textContent = items.length ? `公開時刻: ${{items.join(' / ')}}` : '公開時刻は未設定です';
}}
...
adminAuthButton.addEventListener('click', async () => {{
  ...
  const hashed = await digest(adminPassword.value);
  if (hashed !== adminHash) {{
...
saveSchedule.addEventListener('click', () => {{
  ...
  setTimeout(() => saveMessage.textContent = '', 1400);
}});
...
clearSchedule.addEventListener('click', () => {{
  ...
  updateScheduleSummary();
  applyFilters();
  saveMessage.textContent = '解除しました。';
  setTimeout(() => saveMessage.textContent = '', 1400);
}});
